@extends('admin.layouts.admin')
@section('content')
    <div class="card">
        <div class="card-content">
            <div class="card-body">
                <div class="row">
                    <div class="col-12">
                        <div class="table-responsive text-nowrap ">
                            <table class="table text-center">
                                <thead class="table">
                                <tr>
                                    <th>{{__('panel.table-count')}}</th>
                                    <th>{{__('panel.thump')}}</th>
                                    <th>{{__('panel.title')}}</th>
                                    <th>{{__('panel.description')}}</th>
                                    <th>{{__('museum')}}</th>
                                    <th>{{__('panel.actions')}}</th>
                                </tr>
                                </thead>
                                <tbody class="table-border-bottom-0">
                                @if(!empty($data))
                                    @php
                                        $i = 1 + $data->currentPage() * $data->perPage() - $data->perPage();
                                    @endphp
                                    @foreach($data as $item)
                                        <tr>
                                            <td>
                                                    <span class="fw-medium">
                                                        {{@$i}}
                                                    </span>
                                            </td>
                                            <td>
                                                <div class="avatar-wrapper" style="margin-right: 40%">
                                                    <div class="avatar">
                                                        <img src="/{{@$item->thump->path}}" alt="thump"
                                                             class="rounded-circle">
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                {{@$item->title}}
                                            </td>
                                            <td>
                                                {{@$item->description}}
                                            </td>
                                            <td>
                                                <a class="dropdown-item"
                                                   href="{{route('museum.edit' , ['id' => $item->museum_id])}}"
                                                >{{$item->museum->title}}
                                            </td>

                                            <td>
                                                <div class="dropdown">
                                                    <button type="button" class="btn p-0 dropdown-toggle hide-arrow"
                                                            data-bs-toggle="dropdown">
                                                        <i class="ti ti-dots-vertical"></i>
                                                    </button>
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                           href="{{route('testimonial.edit' , ['id' => $item->id])}}"
                                                        ><i class="ti ti-pencil me-1"></i> {{__('panel.edit')}}</a
                                                        >
                                                        <a class="dropdown-item"
                                                           href="{{route('testimonial.destroy' , ['id' => $item->id])}}"
                                                        ><i class="ti ti-trash me-1"></i> {{__('panel.delete')}}</a
                                                        >
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                                        @php
                                            $i++;
                                        @endphp
                                    @endforeach
                                @endif
                                </tbody>
                            </table>
                        </div>

                    </div>
                    @if(!empty($data) && $data->hasPages())
                        <div class="col-12 text-center mt-5" style="height:50px">
                            {{$data->render()}}
                        </div>
                    @endif
                </div>

            </div>
        </div>
    </div>
    {{--    {{$paginator->count()}}--}}

@stop

@section('script')

@stop
